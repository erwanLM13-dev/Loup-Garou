...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .layout-grid { display: grid; grid-template-columns: 1fr 360px; gap: 0; }
    @media (max-width: 768px) {
      .layout-grid { grid-template-columns: 1fr !important; }
      .layout-sidebar { padding-left: 0 !important; padding-top: 24px; }
      .layout-sidebar > div { position: static !important; }
    }
  `}</style>
);
...
function Layout({ children, sidebar, title, onBack, backLabel = "← Retour" }) {
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      {/* Body */}
      <div style={{ flex: 1, maxWidth: 1400, margin: "0 auto", width: "100%", padding: "32px 16px 60px" }}>
        <div className={sidebar ? "layout-grid" : ""}>
          <main style={{ minWidth: 0 }} className="fade-in">{children}</main>
          {sidebar && <aside className="fade-in layout-sidebar" style={{ paddingLeft: 32 }}>{sidebar}</aside>}
        </div>
      </div>
    </div>
  );
